...
import yoonhuijung.dogcareproject.menu.AfterLogin;
import yoonhuijung.dogcareproject.Interface.ApiConfig;
import yoonhuijung.dogcareproject.Interface.ClickListener;
import yoonhuijung.dogcareproject.RecyclerCollection.adapter.ChattingListAdapter;
import yoonhuijung.dogcareproject.RecyclerCollection.item.ChattingListItem;
...
public class ChattingList extends AppCompatActivity  implements View.OnClickListener, ReceiverThreadmain.OnReceiveListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Call<List<Getchatlists>> call = mapiconfig.getchatlistPersonllay(NAME);
        call.enqueue(new Callback<List<Getchatlists>>() {
            ...
            public void onResponse(Call<List<Getchatlists>> call, Response<List<Getchatlists>> response) {
                ...
                {
                    chattingListItems.add(new Getchatlists(getchatlists.get(i).getSender_image(),getchatlists.get(i).getSender_chat(),getchatlists.get(i).getMsg_chat(),getchatlists.get(i).getSender_time()));
                }
                setRecyclerView();
            }
    ...
    private void getChatlistdataFromSever()
    {
        ...
        call.enqueue(new Callback<List<Getchatlists>>() {
            ...
            public void onResponse(Call<List<Getchatlists>> call, Response<List<Getchatlists>> response) {
                ...
                for(int i=0; i<getchatlists.size(); i++)
                {
                    afterLogin.chattingListItems.add(new Getchatlists(getchatlists.get(i).getSender_image(),getchatlists.get(i).getSender_chat(),getchatlists.get(i).getMsg_chat(),getchatlists.get(i).getSender_time()));
                }
                setRecyclerView();
            }
        ...
        runOnUiThread(new Runnable() {
            ...
            public void run() {
                ...
                for(int i=0; i<chattingListItems.size(); i++)
                {
                    ...
                    {
                        chattingListItems.set(i,new Getchatlists(arrayList.get(arrayList.size()-1).getImage(),arrayList.get(arrayList.size()-1).getSender(),arrayList.get(arrayList.size()-1).getMsg(),arrayList.get(arrayList.size()-1).getMsg()));
                        mAdapter.notifyDataSetChanged();
                    }
                }
            }
