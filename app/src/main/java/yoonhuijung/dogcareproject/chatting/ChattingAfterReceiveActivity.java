...
import yoonhuijung.dogcareproject.RecyclerCollection.adapter.ChattingAdapter;
import yoonhuijung.dogcareproject.get.Getchatlists;
import yoonhuijung.dogcareproject.videocall.CallActivity;
import yoonhuijung.dogcareproject.videocall.ConnectActivity;
import yoonhuijung.dogcareproject.videocall.SSLConnect;
...
public class ChattingAfterReceiveActivity extends AppCompatActivity implements View.OnClickListener, ReceiverThreadmain.OnReceiveListener {

    @BindView(R.id.message_editAfter)
    EditText message_edit;
    @BindView(R.id.send_btnAfter)
    Button send_btn;
    @BindView(R.id.chatRecyclerafter)
    ...
    ArrayList<Getchatlists> arraychatlist;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_chatting_after_receive);
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                try {
                    ...
                    thread2.setOnReceiveListener(ChattingAfterReceiveActivity.this);
                    mThread1.start();
                    thread2.start();
                } catch (IOException e) {
                    e.printStackTrace();
    ...
    void videocallClicked(View veiw)
    {
//        Intent intent = new Intent(this, ConnectActivity.class);
//        startActivity(intent);
        //난수를 생성한다
        Random randomroomnumber = new Random();
        int roomnumber = randomroomnumber.nextInt(100000-10000+1)+10000;
        String roomnumstr = String.valueOf(roomnumber);
        String testroom ="67135";
        SharedPreferences sp = PreferenceManager.getDefaultSharedPreferences(getApplicationContext());
        SharedPreferences.Editor editor = sp.edit();
        editor.putString("방진짜번호",roomnumstr);
        editor.commit();
    ...
    private void  setRecyclerView()
    {
        ...
        for(int i =0; i<arraychatlist.size(); i++)
        {
            chatItemArrayList.add(new Chat_item(arraychatlist.get(i).getSender_chat(),arraychatlist.get(i).getMsg_chat(),false));
        }


        chatRecycler.setLayoutManager(mLayoutManger);
        chatRecycler.setItemAnimator(new DefaultItemAnimator());
        chatRecycler.smoothScrollToPosition(arraychatlist.size());
    ...
    public void onClick(View v) {
        ...
        String name = intent.getStringExtra("발신자");
        onReceiveme(message_edit.getText().toString()+">"+NAME);
        mThread1.sendMessage(NAME+">"+message_edit.getText().toString()+">"+name);
    ...
    public void onReceive(final String message) {
        ...
        Intent intent = getIntent();
        String SenderThatisent = intent.getStringExtra("발신자");
    ...
    private void getCarrerdataFromSever()
    {
        ...
        Call<List<Getchatlists>> call = mapiconfig.getchatPersonsData(name);
        call.enqueue(new Callback<List<Getchatlists>>() {
            @Override
            public void onResponse(Call<List<Getchatlists>> call, Response<List<Getchatlists>> response) {
                List<Getchatlists> getchatlists = response.body();
                for(int i=0; i<getchatlists.size(); i++)
                {
                    arraychatlist.add(new Getchatlists(getchatlists.get(i).getSender_image(),getchatlists.get(i).getSender_chat(),getchatlists.get(i).getMsg_chat(),getchatlists.get(i).getSender_time()));
                }
                setRecyclerView();
            }

            @Override
            public void onFailure(Call<List<Getchatlists>> call, Throwable t) {
